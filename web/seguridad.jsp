...
<c:if test="${standalone eq 'not'}" >
    <c:choose>
        ...
        <c:when test="${pageId eq 'Consultar' and sessionScope.nivel < 0}">
            <c:redirect url='recepcion.jsp'>
                <c:param name="operacion" value="${pageId}"/>
        ...
        <c:when test="${(pageId eq 'Update' or pageId eq 'Delete' or  pageId eq 'FormUpdate')  and sessionScope.nivel != 2}">
            <c:redirect url='recepcion.jsp'>
                <c:param name="operacion" value="${pageId}"/>
                <c:param name="logeado" value="yes"/>
            </c:redirect>
